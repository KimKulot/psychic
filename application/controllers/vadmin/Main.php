<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');
	...
	class Main extends CI_Controller
	{
		...
		function overview($mainNavId = null, $subNavId = null) {
			...
			$packages = $this->db->query("SELECT * FROM `packages`");
			$t['packages'] = $packages->result_array();
		...
		function add_record($mainNavId = null, $subNavId = null, $data = false)
		{
				...
				else
				{
					...
					if (isset($specs['unique'][$mainNavId][$f])) {
						...
						if ($this->db->where($f, $fieldValue)->get($table)->num_rows()) {
							...
							return $this->add_record($mainNavId, $subNavId, $this->input->post());
						}
					}
		...
		function edit_record($mainNavId = null, $subNavId = null, $recordId = null, $data = false)
		{
				...
				else
				{
					...
					if (isset($specs['unique'][$mainNavId][$f])) {
						...
						if ($this->db->where($f, $fieldValue)->where('id !=', $recordId)->get($table)->num_rows()) {
							...
							return $this->edit_record($mainNavId, $subNavId, $recordId, $this->input->post());
						}
					}
