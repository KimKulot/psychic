...
class Base_model extends CI_Model {
        ...
        foreach ($options['where'] as $key => $value) {
            if (is_array($value)) {
                ...
                if ($key == 'or_where') {
                    ...
                    $this->db->where_in($key, $value)->order_by('id', 'desc');
                }
                $this->db->group_end();
            } else {
                $this->db->where($key, $value)->order_by('id', 'desc');
            }
        }
    ...
    {
        $success = false;
        if (isset($this->status) && $this->status == 3) {
            $success = $this->db->update($this->table(), $this->_attrs, ['mobile_num' => $this->mobile_num, 'status !=' => 2]);
        } else {
            $success = $this->db->update($this->table(), $this->_attrs, ['id' => $this->id]);
        }
